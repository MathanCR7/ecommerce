// --- START OF FILE client/src/Pages/Admin/Orders/FailedOrdersPage.jsx ---
...
const FailedOrdersPage = () => {
    ...
    try {
      ...
      setError(
        err.message ||
          ...
          "Failed to fetch failed delivery orders."
      ); // MSG UPDATED
      setOrders([]);
      setFetchedOrders([]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = `failed_delivery_orders_export_${
  ...
  if (showMainSpinner && fetchedOrders.length === 0 && !error)
    content = (
      <div className="order-list-page-container loading">
        <FaSpinner className="spinner-icon spin" /> Loading failed delivery
        orders...
      </div>
    );
    ...
    content = (
      ...
      >
        No failed delivery orders found.
      </div>
    );
...
// --- END OF FILE client/src/Pages/Admin/Orders/FailedOrdersPage.jsx ---
